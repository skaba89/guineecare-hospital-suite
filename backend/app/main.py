...
from app.modules.observability.routes import router as observability_router, metrics_router, k8s_router
...
from app.modules.icd11.routes import router as icd11_router
...
OPENAPI_TAGS = [
    ...
    {"name": "tasks", "description": "Tâches planifiées Celery (prune audit, backup, push DHIS2) — v2.9.2."},
    {"name": "icd11", "description": "Lookup des codes diagnostiques ICD-11 (classification OMS) — v2.9.2."},
...
# v2.2.0 — Phase 6 : méthodes et headers explicites (au lieu de "*") pour
# réduire la surface d'attaque. OPTIONS implicite (géré par CORSMiddleware).
app.add_middleware(
    CORSMiddleware,
    allow_origins=settings.cors_origins,
    allow_credentials=False,
    allow_methods=["GET", "POST", "PUT", "PATCH", "DELETE"],
    allow_headers=["Authorization", "Content-Type", "X-Facility-ID", "X-Bootstrap-Token"],
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # v2.2.0 — Phase 6 : headers supplémentaires (defense-in-depth)
        # HSTS : forcer HTTPS en production (1 an + sous-domaines + preload)
        if settings.environment not in ("local", "test", "dev"):
            response.headers["Strict-Transport-Security"] = (
                "max-age=31536000; includeSubDomains; preload"
            )
        # Permissions-Policy : désactiver les features navigateur non nécessaires
        response.headers["Permissions-Policy"] = (
            "camera=(), microphone=(), geolocation=(), payment=(), usb=()"
        )
        # CSP : autoriser uniquement self pour scripts/styles/img, désactiver
        # les plugins et les frames externes. 'unsafe-inline' nécessaire pour
        # Vite dev server + styles inline des composants React.
...
# v2.9.2 — Tâches Celery (prune audit, backup auto, push DHIS2)
app.include_router(tasks_router, prefix=API_PREFIX)
# v2.9.2 — ICD-11 lookup (catalogue OMS)
app.include_router(icd11_router, prefix=API_PREFIX)
...
# v2.3.0 — Phase 8 : alias Kubernetes /livez et /readyz
...
if _os.path.isdir(_STATIC_DIR):
    # Serve static assets (JS, CSS, images) — cache 1 an (fichiers hashés, immuables)
    _assets_dir = _os.path.join(_STATIC_DIR, "assets")

    class _CachedStaticFiles(_StaticFiles):
        async def get_response(self, path: str, scope):
            response = await super().get_response(path, scope)
            # Les fichiers dans /assets/ sont hashés par Vite → immuables
            response.headers["Cache-Control"] = "public, max-age=31536000, immutable"
            return response

    app.mount("/assets", _CachedStaticFiles(directory=_assets_dir), name="assets")
    ...
    async def serve_spa(full_path: str):
        ...
        if (full_path.startswith("api/") or
            full_path in ("health", "health/live", "health/ready", "metrics", "docs", "redoc",
                          "livez", "readyz", "health/schema")):
            raise HTTPException(status_code=404)
        # Servir le fichier s'il existe, sinon index.html (SPA routing)
        file_path = _os.path.join(_STATIC_DIR, full_path)
        if _os.path.isfile(file_path):
            response = _FileResponse(file_path)
            # v2.8.6 — Cache-Control pour les fichiers statiques non-hashés
            if full_path.endswith(".html"):
                response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
            elif full_path.endswith((".png", ".jpg", ".ico", ".svg", ".webmanifest")):
                response.headers["Cache-Control"] = "public, max-age=86400"
            return response
        # index.html — NE JAMAIS cacher (sinon les vieux hashes JS restent)
        response = _FileResponse(_os.path.join(_STATIC_DIR, "index.html"))
        response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
