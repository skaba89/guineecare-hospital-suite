 ...
 * - Retry automatique sur erreurs réseau / 5xx (GET uniquement, 3 essais,
 *   backoff exponentiel 1s/2s/4s) — P0-4
 * - setOnSessionLost() callback pour synchroniser AuthContext quand la
 *   session est perdue (refresh failure) — P0-5
 */
...
// v2.7.0 — Phase 7 : callback appelé quand la session est perdue (refresh fail)
...
async function refreshAccessToken(): Promise<string | null> {
  ...
  refreshPromise = (async () => {
    try {
      ...
      await clearSession();
      // v2.7.0 — Phase 7 : notifier AuthContext pour synchroniser React state
      if (onSessionLost) {
        try { onSessionLost(); } catch {}
      }
      return null;
    } finally {
      refreshPromise = null;
...
// v2.7.0 — Phase 7 : helper pour déterminer si une erreur est retry-able
...
// v2.7.0 — Phase 7 : helper pour backoff exponentiel (1s, 2s, 4s)
function backoffDelay(retryCount: number): number {
  return Math.min(4000, 1000 * Math.pow(2, retryCount));
}

// --- Response interceptor : refresh on 401 + retry on network/5xx ---
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    const originalRequest = error.config as InternalAxiosRequestConfig & {
      _retried?: boolean;
      _networkRetried?: number;
    };

    // 1. Refresh token sur 401 (une seule fois)
    if (error.response?.status === 401 && !originalRequest._retried && session?.refresh_token) {
      ...
      // Refresh failed → session perdue, propager l'erreur
      return Promise.reject(error);
    }

    // 2. v2.7.0 — Phase 7 : retry sur erreurs réseau / 5xx (GET uniquement)
    // On ne retry que les requêtes idempotentes (GET) pour éviter les
    // doubles écritures. Max 3 essais avec backoff exponentiel.
    const method = (originalRequest.method || 'get').toLowerCase();
    if (method === 'get' && isRetryableError(error)) {
      const retryCount = originalRequest._networkRetried || 0;
      if (retryCount < 3) {
        originalRequest._networkRetried = retryCount + 1;
 ...
 *
 * Seuls first_name et last_name sont obligatoires côté backend.
