...
const TABS: { key: TabKey; label: string }[] = [
  { key: "stock", label: "pharmacy.tab.stock" },
  { key: "dispensation", label: "pharmacy.tab.dispensation" },
  { key: "products", label: "pharmacy.tab.products" },
  { key: "movements", label: "pharmacy.tab.movements" },
];
...
// v2.8.7 — Mock retiré. Les vraies données viendront de /pharmacy/dispensations
// quand l'endpoint de statistiques sera ajouté.
const MOCK_TOP_DISPENSED: { name: string; value: number }[] = [];
  ...
  return (
    <>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (produit)...`}
          value={search}
  ...
  return (
    <>
      ...
      <div className="card form-card">
        <h3 style={{ marginBottom: "16px" }}>{t("pharmacy.dispensation.new")}</h3>
  ...
  return (
    <>
      <div className="section-header">
        <h2>{t("pharmacy.products.title")}</h2>
        <button
          className="primary-button"
          style={{ display: "flex", alignItems: "center", gap: "6px" }}
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("pharmacy.products.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("pharmacy.products.new")}</h3>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (code, nom)...`}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 250, padding: "8px 12px" }}
        />
                ...
                {products.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("pharmacy.products.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  products.map((p) => (
...
function MovementsTab({
  ...
  return (
    <>
      <div className="section-header">
        <h2>{t("pharmacy.movements.title")}</h2>
        <button
          className="primary-button"
          style={{ display: "flex", alignItems: "center", gap: "6px" }}
          onClick={() => setShowForm(!showForm)}
        >
          ...
          {showForm ? t("action.cancel") : t("pharmacy.movements.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("pharmacy.movements.new")}</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <label className="form-control">
                Produit *
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (motif, produit)...`}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 250, padding: "8px 12px" }}
        />
            ...
            <table className="table">
              ...
              <tbody>
                {movements.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("pharmacy.movements.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  movements.map((m) => (
