...
const TABS: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  { key: "tableau", label: "admissions.tab.dashboard", icon: <Activity size={16} /> },
  { key: "nouvelle", label: "admissions.tab.new", icon: <Plus size={16} /> },
  { key: "historique", label: "admissions.tab.history", icon: <Clock size={16} /> },
];
...
// Status → { i18n key, badge class }.
// Labels are translated at the call site via t() so the UI follows the
// active locale (FR default, EN if user switches). Previously these were
// hardcoded English "Active"/"Sorti" which broke FR consistency.
const STATUS_MAP: Record<string, { labelKey: string; badge: string }> = {
  OPEN: { labelKey: "status.active", badge: "badge-green" },
  ACTIVE: { labelKey: "status.active", badge: "badge-green" },
  CLOSED: { labelKey: "status.discharged", badge: "badge-gray" },
  DISCHARGED: { labelKey: "status.discharged", badge: "badge-gray" },
};
    ...
    <>
      ...
      >
        ...
        <KpiCard
          ...
          label={t("admissions.kpi.hospitalized")}
        ...
        <KpiCard
          icon={<LogOut size={20} />}
          label={t("admissions.kpi.discharges")}
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (patient, numéro, motif)...`}
          value={search}
      ...
      >
        <div style={{ display: "grid", gap: "6px", fontWeight: 600, fontSize: "13px" }}>
          <span style={{ color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("label.status")}</span>
          <select
            value={statusFilter}
            onChange={(e) => {
        ...
        <div style={{ display: "grid", gap: "6px", fontWeight: 600, fontSize: "13px" }}>
          <span style={{ color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("label.type")}</span>
          <select
            value={typeFilter}
            onChange={(e) => {
            ...
            style={{ minWidth: "160px", fontSize: "13px" }}
          >
            <option value="">{t("label.all_types")}</option>
        ...
        <div style={{ display: "grid", gap: "6px", fontWeight: 600, fontSize: "13px" }}>
          <span style={{ color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("label.department")}</span>
          <select
            value={deptFilter}
            onChange={(e) => {
          ...
          >
            <option value="">{t("label.all_departments")}</option>
        ...
        <div style={{ display: "grid", gap: "6px", fontWeight: 600, fontSize: "13px" }}>
          <span style={{ color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("label.date_from")}</span>
          <input
            type="date"
            value={dateFrom}
            onChange={(e) => {
        ...
        <div style={{ display: "grid", gap: "6px", fontWeight: 600, fontSize: "13px" }}>
          <span style={{ color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("label.date_to")}</span>
          <input
            type="date"
            value={dateTo}
            onChange={(e) => {
  ...
  return (
    <div className="card" style={{ maxWidth: "900px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
        ...
        <div>
          <h2 style={{ fontSize: "18px", margin: 0 }}>{t("admissions.new.title")}</h2>
          <p className="muted" style={{ fontSize: "13px" }}>
            {t("admissions.new.subtitle")}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-actions">
          <button className="btn btn-primary" type="submit" disabled={submitting}>
            {submitting ? t("admissions.submit.creating") : t("admissions.submit.create")}
          </button>
          <button
  ...
  return (
    <>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (patient, numéro)...`}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 250, padding: "8px 12px" }}
        />
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: "center", padding: "32px" }}>
          <p className="muted">{t("admissions.empty_discharged")}</p>
        </div>
      ) : (
        <div className="card" style={{ padding: "0" }}>
          <div className="table-wrapper" style={{ maxHeight: "520px", overflowY: "auto" }}>
