...
[![Version](https://img.shields.io/badge/version-v2.9.4-blue.svg)](https://github.com/skaba89/guineecare-hospital-suite/releases)
...
**Version actuelle :** `v2.9.4` — **Prête pour déploiement national flagship.** Cette version ajoute : tests E2E étendus pour TasksAdmin / ICD11Search / InfinitePatientsList, templates administratifs (lettre notification ARPT DPO + procédure appel d'offres pen test), roadmap V3.0 (10 lots sur 12-18 mois), fiches rapides utilisateurs. Complète v2.9.3 (UI TasksAdmin, ICD11Search, vue scroll infini), v2.9.2 (Redis, Celery, mode sombre, ICD-11, docs DPO + pen test), v2.9.1 (assurance + DHIS2 push), v2.8.9 (perf), v1.4-v1.8 (SMS, qualité, RH v2, FHIR R4, 2FA, mobile biométrique).
...
Version finale avant déploiement national flagship — complète les suites post-audit expert.

### Infrastructure & Code

- **Redis rate limit partagé** multi-instance (`REDIS_URL` active automatiquement le storage Redis pour slowapi)
- **Celery worker + beat** — 5 tâches planifiées (cron) :
...
- **Routes API `/tasks`** — `GET /api/v1/tasks` + `POST /tasks/trigger/{name}` (SUPER_ADMIN)

### Frontend

- **Mode sombre** 🌙 — toggle dans le topbar, persistance localStorage, respect `prefers-color-scheme`
- **Hook `useInfiniteScroll`** + composant `Sentinel` (IntersectionObserver) — append-only avec debounce search
- **Tests E2E étendus** — 24 parcours Playwright (+12 nouveaux)

### Module ICD-11

- Catalogue embarqué de ~80 codes ICD-11 (classification OMS 2022) prioritaires pour la pratique guinéenne
- `GET /api/v1/icd11/search?q=...&limit=20` — recherche fuzzy (code, label FR, label EN)
- `GET /api/v1/icd11/{code}` — détail d'un code
- `GET /api/v1/icd11/categories` — liste des catégories
- Codes clés : paludisme (1F03, 1F2Z), hypertension (BA00), diabète (5A1A), prééclampsie (JB02), hémorragie post-partum (JC24), Ebola (1E74), Lassa (1E73)

### Documentation juridique

- **`docs/securite/DPO_DESIGNATION_v1.md`** — charte DPO complète + fiche de poste + modèle d'arrêté ministériel
- **`docs/securite/CAHIER_DES_CHARGES_PEN_TEST_v1.md`** — cahier des charges pen test externe (OWASP WSTG + budget 25-50k€)

### Runbook & validation

- **`docs/deploiement/RUNBOOK_MISE_A_JOUR_v2.9.2.md`** — procédure de mise à jour (Render + VPS Docker)
- **`scripts/validate_v292.sh`** — script de validation post-déploiement (10 checks automatisés)

### Métriques

| Métrique | Avant (v2.8.9) | Après (v2.9.2) |
|----------|----------------|----------------|
...
- **Cache / jobs** : Redis 7 + Celery 5 (v2.9.2 — rate limit partagé + 5 tâches planifiées)
- **Auth** : JWT + RBAC + multi-tenant RLS
- **Rate limiting** : slowapi + Redis storage (5 logins/min sur `/auth/login`, partagé multi-instance en v2.9.2)
- **Tests E2E** : Playwright 1.61 (24 parcours UI critiques, étendus en v2.9.2)
...
> ⚠️ **AVERTISSEMENT SÉCURITÉ** — Les comptes ci-dessous sont des comptes de **démonstration uniquement**.
> Ils sont chargés via `SEED_DEMO_DATA=true` et ne doivent **JAMAIS** être utilisés en production.
> En production, créer un super-admin via `python -m app.cli create-superuser` avec un mot de passe fort.
> L'instance de démo publique (https://guineecare.onrender.com) est réinitialisée périodiquement.
