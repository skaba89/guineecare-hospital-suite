...
def test_app_version_matches_current_release(spec):
    """The OpenAPI spec version must match the runtime APP_VERSION.

    The exact version number is intentionally NOT hardcoded here so that
    version bumps (e.g. 1.8.0 → 1.9.0) do not break this test. The contract
    we care about is: spec.info.version == APP_VERSION (already asserted).
    """
    assert spec["info"]["version"] == APP_VERSION
    # Sanity check: version must look like semver (MAJOR.MINOR.PATCH).
    parts = APP_VERSION.split(".")
    assert len(parts) >= 3 and all(p.isdigit() for p in parts[:3]), (
        f"APP_VERSION={APP_VERSION!r} must be semver (MAJOR.MINOR.PATCH)"
    )
