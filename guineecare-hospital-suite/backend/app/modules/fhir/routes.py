...
def search_patients(
    ...
    Sécurité (v2.2.0) : isolation multi-tenant via `tenant_query`.
    Un DOCTOR/MIDWIFE d'un établissement ne peut voir que les patients
    de son établissement. SUPER_ADMIN voit tous.
    """
    # P0 fix v2.2.0 : isolation multi-tenant obligatoire
...
def read_patient(
    ...
    Sécurité (v2.2.0) : `enforce_facility_access` vérifie que le patient
    appartient à l'établissement de l'utilisateur. Audit log de l'accès.
    """
    ...
    # P0 fix v2.2.0 : empêcher la lecture cross-tenant
...
def search_encounters(
    ...
    Sécurité (v2.2.0) : isolation multi-tenant via `tenant_query`.
    """
    # P0 fix v2.2.0 : isolation multi-tenant
    query = tenant_query(db, Admission, current_user)
    if _id:
...
def read_encounter(
    ...
    Sécurité (v2.2.0) : `enforce_facility_access` + audit log.
    """
    ...
    # P0 fix v2.2.0 : empêcher la lecture cross-tenant
    enforce_facility_access(current_user, admission.facility_id)
    audit_log(
        db=db,
...
def search_observations(
    ...
    Sécurité (v2.2.0) : isolation multi-tenant via `tenant_query`.
    LabResult et PatientMeasurement ont tous deux `facility_id`.
    """
    if category == "laboratory":
        # Résultats labo — filtrer par facility_id du LabResult
        query = tenant_query(db, LabResult, current_user)
        ...
        # Constantes vitales — filtrer par facility_id du PatientMeasurement
        query = tenant_query(db, PatientMeasurement, current_user)
        if patient:
            patient_id = patient.split("/")[-1] if "/" in patient else patient
...
def read_observation(
    ...
    Sécurité (v2.2.0) : `enforce_facility_access` sur le facility_id du
    résultat. Audit log de l'accès.
    """
    # Essayer d'abord une constante vitale
    if category != "laboratory":
        m = db.query(PatientMeasurement).filter(PatientMeasurement.id == observation_id).first()
        if m:
            enforce_facility_access(current_user, m.facility_id)
            audit_log(
                db=db,
                action="fhir.observation.read",
                user=current_user,
    ...
    if r:
        enforce_facility_access(current_user, r.facility_id)
        audit_log(
            db=db,
            action="fhir.observation.read",
            user=current_user,
...
def search_medication_requests(
    ...
    """
    # P0 fix v2.2.0 : isolation multi-tenant
    query = tenant_query(db, ClinicalNote, current_user).filter(
        ClinicalNote.note_type == "PRESCRIPTION"
    )
    if _id:
    ...
    if status:
        ...
        # v2.8.2 — P1 fix : ClinicalNote n'a pas de colonne status.
        # Le filtre était dead-code (hasattr retournait False).
        # On filtre sur note_type à la place (toutes les prescriptions sont
        # note_type=PRESCRIPTION, le statut FHIR est dérivé du contenu).
        # Pour une filtration statut réelle, il faudrait une colonne status
        # sur ClinicalNote (futur migration).
        pass
...
def read_medication_request(
    ...
    audit_log(
        db=db,
        action="fhir.medication_request.read",
        user=current_user,
...
def read_diagnostic_report(
    ...
    enforce_facility_access(current_user, result.facility_id)
    audit_log(
        db=db,
        action="fhir.diagnostic_report.read",
        user=current_user,
