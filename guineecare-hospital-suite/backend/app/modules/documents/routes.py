...
def generate_lab_result(
    ...
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    # v2.8.0 — P0-5 fix : gérer les panels (test_id=None)
    # Pour les panels multi-tests, on récupère le premier test du panel
    test = None
    if order.test_id:
        test = db.query(LabTest).filter(LabTest.id == order.test_id).first()
    if not test:
        # Panel : récupérer le premier test via lab_order_tests
        from app.modules.laboratory.models import LabOrderTest
        first_item = (
            db.query(LabOrderTest)
            .filter(LabOrderTest.order_id == order.id)
            .first()
        )
        if first_item:
            test = db.query(LabTest).filter(LabTest.id == first_item.test_id).first()
    if not test:
        raise HTTPException(status_code=404, detail="Aucun test associé à cette commande")
    result = (
        db.query(LabResult)
...
# ── 4b. Payment receipt PDF (v2.4.0 — Phase 4) ────────────────────────
@router.get(
    "/payments/{payment_id}/pdf",
    summary="Générer le PDF d'un reçu de paiement",
    description=(
        "Génère un PDF de reçu pour un paiement spécifique. "
        "Le PDF inclut le n° de reçu, le n° de facture, le mode de paiement, "
        "le montant payé et le solde restant éventuel."
    ),
    response_class=Response,
)
def generate_payment_receipt(
    ...
    """PDF reçu de paiement — v2.4.0 Phase 4."""
    ...
    pdf_bytes, doc_ref, sha = generate_payment_receipt_pdf(
        facility_name=_facility_name(db, payment.facility_id),
        patient=patient,
        payment=payment,
        invoice=invoice,
    )
