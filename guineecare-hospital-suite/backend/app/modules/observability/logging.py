...
def configure_logging(environment: str | None = None) -> None:
    ...
    v2.3.0 — Phase 8 : LOG_LEVEL env var permet de monter en DEBUG sans
    recompile. Valeurs : DEBUG, INFO (défaut), WARNING, ERROR, CRITICAL.
    """
    env = (environment or os.environ.get("ENVIRONMENT", "local")).lower()
    is_json = env in {"production", "staging"}

    # v2.3.0 — Phase 8 : log level configurable via LOG_LEVEL env var.
    # Default INFO. En dev/test, on peut passer LOG_LEVEL=DEBUG pour
    # diagnostiquer sans toucher au code.
    ...
    # Quiet down noisy libs (sauf si LOG_LEVEL=DEBUG explicite)
