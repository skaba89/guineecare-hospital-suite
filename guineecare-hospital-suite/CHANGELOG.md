...
### Suites post-audit — P0 juridique + P1 infrastructure/code + P2 polish

Cette release finalise les éléments restants (hors Render Starter plan)
identifiés par l'audit expert. Le projet est désormais **prêt pour un
déploiement national flagship** sous réserve de la signature formelle
des actes juridiques (DPO, pen test).

#### P0 — Juridique (livrables documentaires)

- **Désignation DPO** : `docs/securite/DPO_DESIGNATION_v1.md`
  - Charte complète du DPO (missions Article 39 RGPD, indépendance Article 38.3)
  - Fiche de poste détaillée (6 missions, KPI annuels)
  - Modèle d'arrêté ministériel de nomination
  - Plan de mise en œuvre en 5 étapes (semaine 1 → mois 6)
  - Référence à la loi guinéenne L/2022/018/AN

- **Cahier des charges pen test externe** : `docs/securite/CAHIER_DES_CHARGES_PEN_TEST_v1.md`
  - Périmètre backend + frontend + mobile (boîte noire / grise / blanche)
  - Méthodologie OWASP WSTG + API Top 10 + Mobile Top 10
  - Règles d'engagement (DoS interdit, exfiltration données réelles interdite)
  - Critères CVSS v3.1 + sur-évaluation pour données de santé
  - Profil prestataire (CREST, OSCP, expérience santé)
  - Budget indicatif : 25 000 € – 50 000 € HT
  - Planning 19 semaines (publication → re-test post-correction)

#### P1 — Infrastructure / Code

- **Redis rate limit partagé multi-instance** (`backend/app/core/redis.py`)
  - Module `get_redis_client()` + `get_rate_limit_storage()` (lazy-import, best-effort)
  - Branchement slowapi storage Redis quand `REDIS_URL` configurée
  - Fallback mémoire (par worker) si Redis absent — pas de dépendance dure
  - Compatible Render multi-instance et Kubernetes replicas

- **Celery worker — tâches planifiées** (`backend/app/tasks/`)
  - `celery_app.py` : Celery app avec beat schedule (4 tâches cron)
  - `maintenance_tasks.py` :
    - `prune_audit_logs` : purge des audit_logs > 365 jours (RGPD Art. 25) — batch 1000
    - `backup_database` : dump PostgreSQL via `pg_dump` (ou copie SQLite en dev) + rotation 30j
    - `retry_sms_pending` : retry des SMS en échec (max age 24h)
  - `reporting_tasks.py` :
    - `push_dhis2_monthly` : push automatique du dataset DHIS2 le 5 du mois (M-1)
    - `send_quality_alerts_digest` : digest quotidien des alertes qualité
  - `routes.py` : `GET /api/v1/tasks` + `POST /api/v1/tasks/trigger/{name}` (SUPER_ADMIN)
  - En dev/test : exécution synchrone (sans worker Celery installé)
  - Compatible Celery absent : `submit_task()` exécute en synchrone

#### P1 — Tests E2E Playwright étendus (`frontend/tests/e2e/v292-extended.spec.ts`)

12 nouveaux parcours E2E couvrant :
- Parcours métier : /pharmacy (PHARMACIST), /lab (LAB_TECH), /billing (CASHIER),
  /maternity, /hospitalization, /imaging, /surgery (DOCTOR)
- Qualité : onglets Dashboard + Alertes (v1.4.0)
- Personnel : /personnel + /planning + /leaves (v1.5.0)
- Pilotage national : /national SUPER_ADMIN + redir DOCTOR
- Activity feed : /activity + RBAC
- SMS admin : /sms-admin + RBAC
- Recherche globale + i18n FR/EN + profil utilisateur
- API Tâches Celery : GET /tasks + POST /tasks/trigger/backup_database (v2.9.2)
- API DHIS2 push : POST /reporting/dhis2/{period}/push (v2.9.1)
- API Insurance : GET /billing/insurance/providers (v2.9.1)

#### P2 — Polish

- **Mode sombre** (`frontend/src/contexts/ThemeContext.tsx` + `ThemeToggle.tsx`)
  - Variables CSS `[data-theme="dark"]` redéfinissent toute la palette
  - Toggle 🌙/☀️ dans le topbar (entre RealtimeStatus et LanguageToggle)
...
- **Infinite scroll** (`frontend/src/hooks/useInfiniteScroll.tsx`)
  - Hook `useInfiniteScroll` + composant `Sentinel` (IntersectionObserver)
  - Append-only : nouvelles pages ajoutées à `items`
  - Debounce search + reset quand `search` ou `extraParams` changent
  - Compatible `apiRequest` existant — pas de refactor des pages

- **ICD-11 lookup** (`backend/app/modules/icd11/`)
  - Catalogue embarqué de ~80 codes ICD-11 (classification OMS 2022)
  - Codes critiques pour la pratique guinéenne : paludisme, hypertension,
    diabète, prééclampsie, hémorragie post-partum, Ebola, Lassa, VIH, TB
  - `GET /api/v1/icd11/search?q=...&limit=20` — recherche fuzzy (code/FR/EN)
  - `GET /api/v1/icd11/{code}` — détail d'un code
  - `GET /api/v1/icd11/categories` — liste des catégories
  - Auth JWT requise (mais pas de permission RBAC — catalogue de référence)

#### Tests backend

- **`test_v292_redis_celery.py`** : 16 tests
  - Redis fallback mémoire (4 tests)
  - Tâches Celery en synchrone (6 tests)
  - Routes /tasks RBAC + exécution (6 tests)

- **`test_v292_icd11.py`** : 22 tests
  - Catalogue (14 tests : recherche, code, catégories, codes guinéens)
  - Routes API (8 tests : search, get, categories, RBAC)

- **Total** : 38 nouveaux tests backend, 0 test existant cassé
- **Total backend** : 280+ tests passent

#### Build frontend

- TypeScript compile sans erreur (tsc --noEmit)
- Vite build OK (dist/ généré)
- Dark mode CSS intégré à styles.css (sans casser le mode clair)
- ThemeToggle monté dans AppLayout topbar
- ThemeProvider wrappé dans main.tsx (entre I18nProvider et App)

#### Mise à jour version

- Backend `app/main.py` : `APP_VERSION = "2.9.2"`
- OpenAPI artifacts régénérés (1 113 204 bytes pour openapi.json)
- Postman collection mise à jour (359 604 bytes)
- Tag OpenAPI `tasks` et `icd11` ajoutés

#### Composants touchés

- Backend :
  - `app/main.py` — version bumpée, 2 routers ajoutés (tasks, icd11)
  - `app/core/limiter.py` — `_build_limiter()` avec Redis storage
  - `app/core/redis.py` — NOUVEAU module Redis client
  - `app/tasks/` — NOUVEAU module (celery_app, maintenance_tasks, reporting_tasks, routes)
  - `app/modules/icd11/` — NOUVEAU module (catalog, routes)
  - `tests/test_v292_redis_celery.py` — NOUVEAU (16 tests)
  - `tests/test_v292_icd11.py` — NOUVEAU (22 tests)
  - `docs/api/openapi.json` — régénéré
  - `docs/api/guineecare.postman_collection.json` — régénéré
  - `docs/securite/DPO_DESIGNATION_v1.md` — NOUVEAU
  - `docs/securite/CAHIER_DES_CHARGES_PEN_TEST_v1.md` — NOUVEAU

- Frontend :
  - `src/contexts/ThemeContext.tsx` — NOUVEAU
  - `src/components/ThemeToggle.tsx` — NOUVEAU
  - `src/hooks/useInfiniteScroll.tsx` — NOUVEAU
  - `src/layout/AppLayout.tsx` — ThemeToggle ajouté au topbar
  - `src/main.tsx` — ThemeProvider wrappé
  - `src/styles.css` — variables CSS dark theme + scrollbars
  - `tests/e2e/v292-extended.spec.ts` — NOUVEAU (12 parcours)

#### État du projet après v2.9.2

| Métrique | Avant (v2.9.0) | Après (v2.9.2) |
|----------|----------------|----------------|
| Version | 2.9.0 | **2.9.2** |
| Tests backend | 243+ | **280+** (+38) |
| Modules backend | 27 | **29** (+tasks, +icd11) |
| Pages/écrans E2E | 12 parcours | **24 parcours** (+12) |
| Mode sombre | ❌ | ✅ |
| Infinite scroll | ❌ | ✅ (hook disponible) |
| ICD-11 | ❌ | ✅ (~80 codes) |
| Redis rate limit | ❌ | ✅ (multi-instance) |
| Celery tasks | ❌ | ✅ (5 tâches, beat schedule) |
| DPO doc | ❌ | ✅ |
| Pen test CDC | ❌ | ✅ |
...
|----------|---------|--------|
| P0 | Render Starter ($7/mois) — élimine cold start | ⏳ Hors scope v2.9.2 |
| P0 | DPO officiellement nommé (arrêté ministériel) | ⏳ Doc prêt, nomination à formaliser |
| P0 | Pen test externe réalisé | ⏳ CDC prêt, prestataire à sélectionner |

Le projet est **opérationnellement prêt** pour un déploiement national.
Les 3 P0 restants relèvent désormais de **décisions administratives**
(signature d'arrêté, lancement d'un appel d'offres, plan Render), pas de
développement code supplémentaire.
