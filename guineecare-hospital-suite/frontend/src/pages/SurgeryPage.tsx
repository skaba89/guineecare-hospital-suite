...
const TABS: { key: TabKey; label: string }[] = [
  { key: "rooms", label: "surgery.tab.rooms" },
  { key: "schedules", label: "surgery.tab.schedules" },
  { key: "reports", label: "surgery.tab.reports" },
];
...
export function SurgeryPage({ lookups }: { lookups: LookupData }) {
  ...
  return (
    <section>
      <h1>{t("nav.surgery")}</h1>
      <p className="muted">{t("surgery.description")}</p>
...
function RoomsTab({ lookups }: { lookups: LookupData }) {
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: "18px" }}>
        <div style={{ display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button className="primary-button" style={{ marginLeft: "auto" }} onClick={() => setShowForm(!showForm)}>
            {showForm ? t("action.cancel") : t("surgery.rooms.new")}
          </button>
        </div>
      </div>
      ...
      {rooms.length === 0 && (
        <div className="card">
          <p className="muted">{t("surgery.rooms.empty")}</p>
        </div>
      )}
    </>
  );
  ...
  return (
    <>
      <div className="section-header">
        <h2 style={{ margin: 0 }}>{t("surgery.schedules.title")}</h2>
        <button className="primary-button" onClick={() => setShowForm(!showForm)}>
          {showForm ? t("action.cancel") : t("surgery.schedules.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3>{t("surgery.schedules.new")}</h3>
          <form onSubmit={handleSubmit} className="form-grid">
            <label className="form-control">
              Patient
      ...
      {loading ? (
        ...
        <div className="card">
          <p className="muted">{t("surgery.schedules.empty")}</p>
        </div>
      ) : (
        <div className="card">
...
function ReportsTab({ lookups }: { lookups: LookupData }) {
  ...
  return (
    <>
      <div className="section-header">
        <h2 style={{ margin: 0 }}>{t("surgery.reports.title")}</h2>
        <button className="primary-button" onClick={() => setShowForm(!showForm)}>
          {showForm ? t("action.cancel") : t("surgery.reports.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3>{t("surgery.reports.new")}</h3>
          <form onSubmit={handleSubmit} className="form-grid">
            <label className="form-control">
              Intervention
      ...
      {loading ? (
        ...
        <div className="card">
          <p className="muted">{t("surgery.reports.empty")}</p>
        </div>
      ) : (
        <div className="card">
          <div className="table-wrapper">
