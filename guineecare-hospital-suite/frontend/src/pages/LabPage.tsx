...
const TABS: { key: TabKey; label: string }[] = [
  { key: "dashboard", label: "lab.tab.dashboard" },
  { key: "orders", label: "lab.tab.orders" },
  { key: "results", label: "lab.tab.results" },
  { key: "catalog", label: "lab.tab.catalog" },
];
  ...
  return (
    <>
      <div className="section-header">
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("lab.orders.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("lab.orders.new")}</h3>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (patient, test, n° demande)...`}
          value={search}
                ...
                {orders.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("lab.orders.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  orders.map((order) => {
                        ...
                        <td>
                          <div style={{ display: "flex", gap: "6px" }}>
                            ...
                            {order.status === "IN_PROGRESS" && (
                              ...
                              >
                                <CheckCircle2 size={12} /> {t("action.complete")}
                              </button>
                            )}
                            <PdfButton
  ...
  return (
    <>
      <div className="section-header">
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("lab.results.new")}
        </button>
      </div>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (patient, test, résultat)...`}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 250, padding: "8px 12px" }}
        />
                ...
                {results.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("lab.results.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  results.map((r) => {
                      ...
                      >
                        ...
                        <td>
                          {r.status === "DRAFT" && (
                            ...
                            >
                              <FileCheck size={12} /> {t("action.validate")}
                            </button>
                          )}
                        </td>
                      </tr>
  ...
  return (
    <>
      <div className="section-header">
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("lab.catalog.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("lab.catalog.new")}</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <label className="form-control">
                Code *
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (code, nom, catégorie)...`}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 250, padding: "8px 12px" }}
        />
                ...
                {tests.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("lab.catalog.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  tests.map((t) => (
