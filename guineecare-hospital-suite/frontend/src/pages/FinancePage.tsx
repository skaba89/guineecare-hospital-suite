...
const TABS: { key: TabKey; label: string }[] = [
  { key: "dashboard", label: "finance.tab.dashboard" },
  { key: "invoices", label: "finance.tab.invoices" },
  { key: "payments", label: "finance.tab.payments" },
  { key: "tariffs", label: "finance.tab.tariffs" },
];
...
// v2.8.0 — MOCK retiré. Les données réelles viennent de /billing/dashboard.
// Le graphique de tendance sera alimenté par les paiements réels quand
// l'endpoint /billing/revenue-trend sera ajouté (Phase 9).
const MOCK_REVENUE_30D: {date: string; revenue: number}[] = [];

// v2.8.7 — Mock retiré. Les vraies données viendront de /billing/dashboard
// quand l'endpoint de répartition par service sera ajouté.
const MOCK_SERVICE_DISTRIBUTION: { name: string; value: number }[] = [];
  ...
  return (
    <>
      <div className="section-header">
        <h2>{t("finance.invoices.title")}</h2>
        <button
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("finance.invoices.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("finance.invoices.new")}</h3>
      ...
      >
        <input
          type="text"
          placeholder={`🔍 ${t("action.search")} (n° facture, description)...`}
          value={search}
                ...
                {invoices.length === 0 ? (
                  <tr>
                    <td colSpan={8} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("finance.invoices.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  invoices.map((inv) => {
...
function PaymentsTab({
  ...
  return (
    <>
      <div className="section-header">
        <h2>{t("finance.payments.title")}</h2>
        <button
          className="primary-button"
          style={{ display: "flex", alignItems: "center", gap: "6px" }}
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("finance.payments.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("finance.payments.new")}</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <label className="form-control">
                Facture *
                ...
                {payments.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("finance.payments.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  payments.map((p) => (
  ...
  return (
    <>
      <div className="section-header">
        ...
        >
          <Plus size={16} />
          {showForm ? t("action.cancel") : t("finance.tariffs.new")}
        </button>
      </div>

      {showForm && (
        <div className="card form-card">
          <h3 style={{ marginBottom: "16px" }}>{t("finance.tariffs.new")}</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <label className="form-control">
                Code *
            ...
            <table className="table">
              ...
              <tbody>
                {tariffs.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "24px" }}>
                      <span className="muted">{t("finance.tariffs.empty")}</span>
                    </td>
                  </tr>
                ) : (
                  tariffs.map((t) => (
