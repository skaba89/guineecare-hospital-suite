...
const TABS: { key: TabKey; label: string }[] = [
  { key: "records", label: "maternity.tab.records" },
  { key: "new_record", label: "maternity.tab.new_record" },
  { key: "consultations", label: "maternity.tab.consultations" },
  { key: "deliveries", label: "maternity.tab.deliveries" },
];
...
export function MaternityPage({ lookups }: { lookups: LookupData }) {
  ...
  return (
    <section>
      <h1>{t("nav.maternity")}</h1>
      <p className="muted">{t("maternity.description")}</p>
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="card">
          <p className="muted">{t("maternity.records.empty")}</p>
        </div>
      ) : (
        <div className="card">
    ...
    <>
      <div className="section-header">
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          <h2 style={{ margin: 0 }}>{t("maternity.consultations.title")}</h2>
        ...
        {currentRecord && (
          <button className="primary-button" onClick={() => setShowForm(!showForm)}>
            {showForm ? t("action.cancel") : t("maternity.consultations.new")}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="card form-card">
          <h3>{t("maternity.consultations.new")}</h3>
          <form onSubmit={handleSubmit} className="form-grid">
            <label className="form-control">
...
function DeliveriesTab({
  ...
  return (
    <>
      <div className="section-header">
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          <h2 style={{ margin: 0 }}>{t("maternity.deliveries.title")}</h2>
          <label className="toolbar-control" style={{ marginBottom: 0 }}>
            <select
              value={currentRecord}
              onChange={(e) => setCurrentRecord(e.target.value)}
        ...
        {currentRecord && (
          <button className="primary-button" onClick={() => setShowForm(!showForm)}>
            {showForm ? t("action.cancel") : t("maternity.deliveries.new")}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="card form-card">
          <h3>{t("maternity.deliveries.new")}</h3>
          <form onSubmit={handleSubmit} className="form-grid">
            <label className="form-control">
              Date d'accouchement
