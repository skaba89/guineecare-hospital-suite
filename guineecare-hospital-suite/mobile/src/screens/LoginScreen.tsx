 ...
 * - Affichage des erreurs serveur (messages FR compréhensibles)
 ...
 *
 * v2.8.0 — Phase 7 : credentials démo retirés du bundle production.
 * Les credentials ne doivent jamais être hardcodés dans l'app mobile —
 * ils sont extractibles du bundle APK et constituent une faille de sécurité.
 * En dev, utiliser EXPO_PUBLIC_DEV_EMAIL / EXPO_PUBLIC_DEV_PASSWORD env vars.
 */
...
export function LoginScreen() {
  ...
  // v2.8.0 — Phase 7 : credentials démo via env vars (DEV uniquement)
  // En production, EXPO_PUBLIC_DEV_EMAIL n'est pas défini → bouton invisible.
  const devEmail = process.env.EXPO_PUBLIC_DEV_EMAIL;
  const devPassword = process.env.EXPO_PUBLIC_DEV_PASSWORD;
  const isDev = !!devEmail && !!devPassword;
      ...
      >
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.footer}>
            <Text style={styles.footerText}>v2.8.0 · CHU Donka · Ministère de la Santé</Text>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
